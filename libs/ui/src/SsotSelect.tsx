...
// Inline stroke icons (chevron-down + check) so @ssot/ui carries zero runtime
// deps beyond react. Geometry matches the lucide/tabler equivalents the apps
// used before; they inherit currentColor from the .ssot-select CSS classes.
function ChevronDownIcon({ size }: { size: number }) {
  return (
    <svg
      className="ssot-select-arrow"
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
...
/**
 * Canonical SSOT listbox: a custom-rendered replacement for native <select> so
 * the OPEN dropdown matches the .ssot-listbox panel across every app. The
 * trigger is styled like .ssot-select-trigger (compact "pill" variant for joint
 * rows); the panel is portaled to <body> and positioned with fixed coordinates
 * so it escapes the scroll/overflow clipping of the surrounding panels.
 *
 * This is the single source shared via @ssot/ui — the "use client" directive is
 * harmless under Vite and required for Next.js server components.
 */
  ...
  return (
    <>
      ...
      >
        ...
        <ChevronDownIcon size={variant === "pill" ? 13 : 15} />
      </button>
      {open &&
        pos &&
        createPortal(
