...
//
// The app is mounted under a basePath by the gateway. Next auto-prefixes the
// /api rewrite source with the basePath, but client-side fetches are NOT
// prefixed automatically — so prepend NEXT_PUBLIC_BASE_PATH here (inlined at
// build) to every request. NEXT_PUBLIC_API_BASE still overrides when set.
const API_BASE = process.env.NEXT_PUBLIC_API_BASE ?? process.env.NEXT_PUBLIC_BASE_PATH ?? "";
...
export type Variant = {
  ...
  // Authoritative eval harness ("isaac"/"dexjoco") from the same registry,
  // derived from the model's eval body script; null when the MODEL_ID is
  // unknown. Replaces the frontend's EVAL_HARNESS-var heuristic.
  harness?: string | null;
};
