...
export function LoadingState({
  ...
  return (
    <div className={cn("space-y-3", className)} aria-busy="true">
      <p className="text-sm text-[var(--ssot-text-soft)]">{label}</p>
...
export function InlineLoading({
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1.5 text-xs text-[var(--ssot-text-soft)]",
        className,
      )}
      aria-busy="true"
    >
...
export function EmptyState({
  ...
  return (
    <p className={cn("text-sm text-[var(--ssot-text-soft)]", className)}>
      {message}
    </p>
  );
}
