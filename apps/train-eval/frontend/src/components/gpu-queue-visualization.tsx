...
export function GpuQueueTooltipContent({
  ...
  if (!snapshot) {
    return <span className="font-mono text-xs text-[var(--ssot-text-soft)]">queue unavailable</span>;
  }
    ...
    <div className="space-y-1 font-mono text-xs leading-snug">
      {snapshot.nodes.map((node) => (
        <div key={node.name} className="grid grid-cols-[4.5rem_3rem_auto_2.5rem_4rem] items-center gap-2">
          <span className="text-[var(--ssot-text-soft)]" title={node.name}>{nodeLabel(node.name)}</span>
          <span>{node.gpu_type ?? "GPU"}</span>
          <span className="tracking-wider">{gpuSquares(node.gpu_used, node.gpu_total)}</span>
          <span className="text-right text-[var(--ssot-text-soft)]">
      ...
      <div className="border-t border-slate-200 pt-1 dark:border-slate-800">
        <span className="text-[var(--ssot-text-soft)]">gpu queue ({snapshot.queue.length})</span>{" "}
...
function nodeStateClassName(state: string | null) {
  ...
  return "text-[var(--ssot-text-soft)]";
}
