  ...
  return (
    <>
      ...
      {showEpisodes && d.episodes !== null && (
        <span className="ml-1 text-xs text-[var(--ssot-text-soft)]">
          · {d.episodes} ep
        </span>
      )}
    </>
    ...
    <>
      <ImmediateTooltip content={`Dataset directory: ${datasetDir}`}>
        <button
          ...
          className="ml-2 inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-xs text-[var(--ssot-text-soft)] transition-colors hover:bg-[var(--ssot-accent-soft)] hover:text-[var(--ssot-accent)]"
        >
...
function MultiRowPicker({
  ...
  return (
    <div className="space-y-1.5">
        ...
        {values.map((value, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            <ImmediateTooltip content="Remove">
              <button
                onClick={() => removeRow(i)}
                className="rounded p-1.5 text-[var(--ssot-text-faint)] transition-colors hover:bg-[var(--ssot-danger-soft)] hover:text-[var(--ssot-danger)]"
              >
      ...
      <p className="text-xs text-[var(--ssot-text-soft)]">{help}</p>
    </div>
  );
}
