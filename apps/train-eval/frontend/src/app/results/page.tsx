...
// The results-sheet viewer is mounted on the same gateway origin at /results.
// That path is base-path-independent (it is a sibling mount, not under this
// app's basePath). Override with NEXT_PUBLIC_RESULTS_VIEWER_URL.
function resultsViewerUrl(): string {
  return process.env.NEXT_PUBLIC_RESULTS_VIEWER_URL ?? "/results";
}
    ...
    <div className="mx-auto max-w-7xl px-8 py-12">
      <div className="flex items-baseline justify-between gap-4">
        <div>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2 text-sm text-[var(--ssot-text-soft)]">
        ...
        <div className="flex flex-wrap items-end justify-end gap-3">
          <div className="space-y-1">
            <Label htmlFor="result-name" className="text-xs text-[var(--ssot-text-soft)]">Name</Label>
            <Input
              id="result-name"
              value={nameFilter}
      ...
      {!initialLoading && anyLoaded && probing.length === 0 && allVariants.length === 0 && (
        <Card className="mt-8">
          <CardContent className="flex items-center gap-3 py-8 text-sm text-[var(--ssot-text-soft)]">
        ...
        <span className="flex min-w-0 items-center gap-2">
          ...
          {states.map(([state, count]) => (
            <span key={state} className="inline-flex shrink-0 items-center gap-0.5">
              ...
              {count > 1 && <span className="text-xs text-[var(--ssot-text-soft)]">×{count}</span>}
            </span>
          ))}
        </span>
  ...
  return (
    <Card className={className}>
      <CardHeader className="gap-3 sm:flex-row sm:items-start sm:justify-between sm:space-y-0">
        <div className="min-w-0">
          ...
          {variant.completed_at != null && (
            <div className="mt-1 text-xs text-[var(--ssot-text-soft)]">
      ...
      <CardContent>
        ...
        <div className="mt-4 grid gap-2 text-xs text-[var(--ssot-text-soft)] md:grid-cols-2">
          {variant.checkpoint && (
            <Meta
              label="checkpoint"
  ...
  return (
    <span className="inline-flex max-w-full min-w-0 items-center gap-2">
      <Link
        ...
        target="_blank"
        rel="noreferrer"
        className="inline-flex min-w-0 items-center gap-1.5 text-[var(--ssot-accent)] hover:underline"
  ...
  return (
    <tr className="border-b border-slate-100 last:border-0 dark:border-slate-900">
      <td className="min-w-[260px] py-3 pr-4 align-top">
        ...
        {task.instruction && (
          <div className="mt-1 max-w-[520px] text-xs text-[var(--ssot-text-soft)]">
            {task.instruction}
          </div>
        )}
      </td>
  ...
  return (
    <ImmediateTooltip content={tooltip} className="inline-flex">
      <div>
        ...
        <div className="mt-1 flex items-center gap-2 text-xs text-[var(--ssot-text-soft)]">
          ...
          {incomplete && <Badge variant="warning" className="px-1 py-0 text-xs">partial</Badge>}
        </div>
      </div>
    </ImmediateTooltip>
  );
