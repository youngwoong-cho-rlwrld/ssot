      ...
      <div className="mt-4 flex items-baseline justify-between gap-4">
        <div>
          <h1 className="text-xl font-semibold tracking-tight">
            ...
            {resumeOf && (
              <span className="text-base font-normal text-[var(--ssot-text-soft)]">
              ...
              {isEval && details.data.training_job && (
                <div className="flex items-center gap-1.5 text-sm text-slate-600 dark:text-slate-400">
                  ...
                  {details.data.training_job.job_name && (
                    <span className="truncate font-mono text-xs text-[var(--ssot-text-soft)]">
                      {details.data.training_job.job_name}
                    </span>
                  )}
                </div>
                ...
                {["State", "ExitCode", "Elapsed", "Start", "End", "Partition", "NodeList", "GPUs", "Reason"].map((k) =>
                  sacct.data[k] ? (
                    <div key={k} className="flex flex-col">
                      <dt className="text-xs uppercase tracking-wide text-[var(--ssot-text-soft)]">{k}</dt>
      ...
      <CardContent className="space-y-4">
        ...
        {!isLoading && !error && d && isTrain && (snapshot || shownWandbProject) && (
          <>
            ...
            {snapshot?.text && (
              <pre className="max-h-80 overflow-auto rounded border border-[var(--ssot-border)] bg-[var(--ssot-surface-muted)] p-3 text-xs">
                {snapshot.text}
              </pre>
            )}
          </>
...
function SnapshotRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-4 py-2">
      <div className="min-w-[110px] text-xs uppercase tracking-wide text-[var(--ssot-text-soft)]">
        {label}
      </div>
...
function GpuUsageSection({
  ...
  return (
    <div className="mt-5 border-t border-slate-100 pt-4 dark:border-slate-900">
      ...
      {!isLoading && error && <ErrorState message={error.message} />}
      {!isLoading && !error && !gpu && (
        <p className="text-sm text-[var(--ssot-text-soft)]">GPU sample unavailable.</p>
      )}
      {!isLoading && !error && gpu && !hasUsage && (
        <div className="space-y-1 text-sm text-[var(--ssot-text-soft)]">
          <p>{gpu.error ?? "GPU utilization is unavailable."}</p>
          {gpu.node && <p className="font-mono text-xs">node: {gpu.node}</p>}
        </div>
      )}
      {!isLoading && !error && gpu && hasUsage && (
        <div className="space-y-3 text-xs text-[var(--ssot-text-soft)]">
          ...
          <>
            ...
            {!showBar && wandbMissing && (
              <p className="text-sm text-slate-600 dark:text-slate-400">
                <a
                  href={process.env.NEXT_PUBLIC_SSOT_SETTINGS_URL ?? "/settings"}
            ...
            {!progressLoading && !progressError && !showBar && !wandbMissing && (
              <p className="text-sm text-[var(--ssot-text-soft)]">{p?.current_label ?? "No progress yet."}</p>
            )}
            {!progressError && showBar && (
              <>
                <div className="flex items-baseline justify-between text-sm">
                  <span className="font-mono">{label}</span>
                  <span className="text-[var(--ssot-text-soft)]">{effectivePercent.toFixed(1)}%</span>
                </div>
                <ProgressBar percent={effectivePercent} height="h-2" className="mt-2" />
                {eta && (
                  <ImmediateTooltip content={eta.etaTitle}>
                    <div className="mt-2 text-xs text-[var(--ssot-text-soft)]">
                      ~{eta.etaLabel} left
                    </div>
                  </ImmediateTooltip>
                )}
...
function PathsCard({
  ...
  return (
    <Card className="mt-6">
      <CardHeader>
        <CardTitle>Paths <span className="text-xs font-normal text-[var(--ssot-text-soft)]">on {d?.cluster ?? cluster}</span></CardTitle>
      </CardHeader>
      <CardContent>
        ...
        {!isLoading && !error && d && (
          <div className="divide-y divide-slate-100 dark:divide-slate-900">
            {rows.map((r) => (
              <div key={r.label}>
                <div className="flex items-center justify-between gap-4 py-2">
                  <div className="min-w-[110px] text-xs uppercase tracking-wide text-[var(--ssot-text-soft)]">{r.label}</div>
                  <div
                  ...
                  {r.label === "checkpoints" && checkpointUnavailable && (
                    <span className="shrink-0 text-xs text-[var(--ssot-text-soft)]">
        ...
        {!isLoading && !error && rows.length > 0 && (
          <div className="overflow-x-auto">
            <table className="min-w-[900px] w-full table-fixed text-sm">
              ...
              <thead className="border-b border-slate-200 text-left text-xs uppercase tracking-wide text-[var(--ssot-text-soft)] dark:border-slate-800">
                <tr>
                  {hasTask && <Th>Task</Th>}
                  <Th>Set</Th>
                  <Th>Run</Th>
