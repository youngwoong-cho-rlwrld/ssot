    ...
    <Card>
      <CardHeader>
        <div className="flex items-baseline justify-between">
          <CardTitle className="text-base">{cluster} <span className="ml-1 text-xs font-normal text-[var(--ssot-text-soft)]">slurm</span></CardTitle>
                ...
                {ps.map((p) => (
                  ...
                  >
                    <td className="py-2 pr-4 font-mono text-xs">
                      {p.name}
                      {p.is_default && <Badge variant="secondary" className="ml-1 text-xs">default</Badge>}
                    </td>
                    <td className="py-2 pr-4 font-mono text-xs">
                      <span className={p.gpu_free > 0 ? "text-green-600 dark:text-green-400" : "text-[var(--ssot-text-soft)]"}>
                        {p.gpu_free}
                      </span>
                    ...
                    <td className="py-2 pr-4 text-xs">
                      {Object.entries(p.states).map(([st, n]) => (
                        <span key={st} className="mr-2 font-mono">
                          <span className={st === "idle" ? "text-green-600 dark:text-green-400" : "text-[var(--ssot-text-soft)]"}>{st}</span>:{n}
                        </span>
                      ))}
                    </td>
                  </QueueTooltipRow>
...
function MlxpPanel() {
  ...
  return (
    <Card>
      <CardHeader>
        <div className="flex items-baseline justify-between">
          <CardTitle className="text-base">MLXP <span className="ml-1 text-xs font-normal text-[var(--ssot-text-soft)]">naver, k8s</span></CardTitle>
          <GpuAvailabilitySummary available={available} total={total} queued={queued} />
        </div>
      </CardHeader>
      <CardContent>
              ...
              <tbody>
                {nodes.map((n) => (
                  ...
                  >
                    ...
                    <td className="py-2 pr-4 font-mono text-xs">
                      <span className={n.gpu_free > 0 ? "text-green-600 dark:text-green-400" : "text-[var(--ssot-text-soft)]"}>
                        {n.gpu_free}
                      </span>
...
function GpuAvailabilitySummary({
  ...
  return (
    <span className="font-mono text-sm">
      <span className={available > 0 ? "text-green-600 dark:text-green-400" : "text-[var(--ssot-text-soft)]"}>{available}</span>
      <span className="text-slate-400"> / {total} GPU available</span>
      <span className="text-slate-400"> · </span>
      <span className={queued > 0 ? "text-amber-600 dark:text-amber-400" : "text-[var(--ssot-text-soft)]"}>{queued}</span>
      ...
      {visible && typeof document !== "undefined"
        ? createPortal(
            <span
              ref={tooltipRef}
              role="tooltip"
              className="pointer-events-none fixed z-[1000] w-max max-w-[min(32rem,calc(100vw-2rem))] whitespace-normal rounded-md border border-[var(--ssot-border)] bg-[var(--ssot-surface)] px-3 py-2 text-xs font-normal leading-snug text-[var(--ssot-text)] shadow-lg [overflow-wrap:anywhere]"
