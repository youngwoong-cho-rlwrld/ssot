...
export default function SubmitPage() {
  ...
  const isDexjoco = evalHarness(variant.data?.harness) === "dexjoco";
  ...
  const evalConfigDefaults = useMemo(
    () => ({
      ...
      dexjocoTask:
        (variant.data?.arrays.TASKS ?? [])
          .map((e) => e.split("|"))
          .find((p) => p[0] === "__single__")?.[1] ?? "",
    }),
    [variant.data],
  );
  ...
  // A multi-task variant has TASKS rows beyond the lone __single__ sentinel:
  // the eval harness runs every task in one job, so no single task is picked
  // (or required). A single __single__ row is a single-task variant.
  const isMultitask = (variant.data?.arrays.TASKS ?? []).some(
    (e) => e.split("|", 1)[0] !== "__single__",
  );
              ...
              >
                <SelectTrigger className="gap-2">
                  ...
                  {dexjocoTasks.isFetching && (
                    <Loader2 className="h-3.5 w-3.5 shrink-0 animate-spin text-[var(--ssot-text-soft)]" />
                  )}
                </SelectTrigger>
                <SelectContent>
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    {filteredVariantNames.length === 0 && (
                      <div className="px-3 py-2 text-sm text-[var(--ssot-text-soft)]">
                      ...
                      {partitions.data?.map((p) => (
                        <SelectItem key={p.name} value={p.name}>
                          <span className="flex items-center gap-2">
                            ...
                            <span className="ml-2 text-xs text-[var(--ssot-text-soft)]">
                      ...
                      {mlxp.data?.map((n) => (
                        <SelectItem key={n.name} value={n.name}>
                          <span className="flex items-center gap-2">
                            ...
                            <span
                              className={`text-xs ${n.gpu_free > 0 ? "text-green-600 dark:text-green-400" : "text-[var(--ssot-text-soft)]"}`}
                            >
                ...
                {gitCommitOptions.data?.commits.map((commit) => (
                  ...
                  >
                    <span className="flex min-w-0 max-w-full items-center gap-2 overflow-hidden">
                      ...
                      <span
                        className="min-w-0 truncate text-xs text-[var(--ssot-text-soft)]"
          ...
          return (
            <button
              ...
              className={[
                ...
                active
                  ? "text-[var(--ssot-text)]"
                  : "text-[var(--ssot-text-faint)] hover:bg-[var(--ssot-accent-soft)] hover:text-[var(--ssot-text-soft)]",
              ...
              <span
                className={[
                  ...
                  active
                    ...
                    : "bg-[var(--ssot-surface-muted)] text-[var(--ssot-text-faint)]",
                ].join(" ")}
              >
                {index + 1}
              </span>
  ...
  return (
    <Field
      label={
        <span className="flex items-center gap-2">
          Train note
          <span className="font-mono text-xs font-normal text-[var(--ssot-text-soft)]">
            {value.length}
          </span>
        </span>
      }
...
function JobNameField({
  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-[var(--ssot-text-soft)]">{description}</p>
    </Field>
  );
}
...
function NumberCellEditor({
  ...
  return (
    <div className="space-y-1">
      ...
      {valid && hint && (
        <p className="text-xs text-[var(--ssot-text-soft)]">{hint}</p>
      )}
    </div>
  );
}
