# Experiment: physixel_poc1_pt48
# PhysiXel PoC1 full per-dim state tokens (K=48) on 3 V4 sim datasets at 480x640, AH16.
...
# K=48 is the full state dim, so every token holds exactly one state dim.
# Explicit identity grouping (dim i -> token i, dataset info.json order) is used
# instead of random_balanced because random_balanced would still shuffle the
# dim->token assignment by seed; explicit makes randomization impossible and
# the seed unused. No ps variants exist since only one composition is possible.
TRAIN_EXTRA_ARGS=(
    --state-part-mode
    explicit
...
TRAIN_NOTE="PoC1 full per-dim state tokens K=48 explicit identity (no randomization); PhysiXel feature/physixel-poc1-part-state-tokens 9f731f9, AH16"
