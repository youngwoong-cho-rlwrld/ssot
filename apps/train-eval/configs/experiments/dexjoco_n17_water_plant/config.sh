# Experiment: dexjoco_n17_water_plant
# DexJoCo water_plant - n17 (single-arm). Train + eval; pick the phase at submit.

# ───── model ─────
MODEL_ID=dexjoco-n17
TRAIN_MODALITY_CONFIG=dexjoco_config.py            # n1.7 modality config (single launcher --modality-config-path)
TRAIN_ACTION_HORIZON=16
TRAIN_NOTE="DexJoCo water_plant - n17 (single-arm)"
