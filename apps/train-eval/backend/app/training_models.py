...
SUPPORTED_FAMILIES = frozenset({"n1.5", "n1.6", "n1.7", "gam"})  # training families flags.py + variants.py know how to render; pi0.5 is eval-only (dexjoco harness)

# Eval harness is a property of the model, not the experiment: it follows 1:1
# from the model's eval body script. Adding a harness/body means one entry here.
_HARNESS_BY_EVAL_BODY = {
    "eval_body.sh": "isaac",
    "eval_body_dexjoco.sh": "dexjoco",
}
...
def load_training_model(model_id: str) -> TrainingModel:
    ...
    harness = _HARNESS_BY_EVAL_BODY.get(eval_body_script)
    if harness is None:
        raise ValueError(
            f"model {model_id}: no harness mapped for EVAL_BODY_SCRIPT {eval_body_script!r}"
        )
