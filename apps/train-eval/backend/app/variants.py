...
def dataset_names_for(variant: Variant) -> list[str]:
    """Train dataset names for filtering, across the config shapes we support.

    N1.6/GAM use TRAIN_DATASET_NAMES=(name ...); N1.5 uses
    DATASETS=("name|cfg|weight" ...); single-dataset variants set DATASET_NAME.
    """
    names = variant.arrays.get("TRAIN_DATASET_NAMES")
    if names:
        return list(names)
    datasets = variant.arrays.get("DATASETS")
    if datasets:
        return [entry.split("|", 1)[0] for entry in datasets if entry]
    single = (variant.vars.get("DATASET_NAME") or "").strip()
    return [single] if single else []


async def list_variant_summaries() -> list[VariantSummary]:
    """Per-variant filter facets (MODEL_ID + train dataset names).

    Parses every config.sh (bounded by the module-level bash semaphore) so the
    experiments list can filter client-side without a round trip per variant.
    """
    async def summarize(name: str) -> VariantSummary:
        variant = await load_variant(name)
        try:
            family = resolve_training_model(variant).family
        except Exception:
            family = None
        return VariantSummary(
            name=name,
            model_id=(variant.vars.get("MODEL_ID") or None),
            model_family=family,
            dataset_names=dataset_names_for(variant),
        )

    return list(await asyncio.gather(*(summarize(n) for n in list_variants())))
