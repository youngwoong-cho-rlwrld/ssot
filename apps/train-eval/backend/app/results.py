...
async def _variant_payload() -> list[dict[str, Any]]:
    ...
    for v in loaded:
        ...
        try:
            model_label = resolve_training_model(v).label
        except Exception:
            model_label = v.vars.get("MODEL_ID") or v.vars.get("MODEL_VERSION")
        payload.append(
            {
                "variant": v.name,
...
def _tasks_for_variant(v: variants.Variant) -> list[dict[str, str | None]]:
    ...
    # A lone __single__ sentinel row is a single-task variant: expose it under
    # the variant name (matching the pre-TASKS payload) rather than leaking the
    # sentinel short, while keeping the row's task_name/instruction.
    if len(out) == 1 and out[0]["short"] == "__single__":
        out[0]["short"] = v.name
        return out
    if out:
        return out
    return [
        {
            "short": v.name,
            "task_name": v.vars.get("TASK_NAME") or v.vars.get("DEXJOCO_TASK") or v.name,
