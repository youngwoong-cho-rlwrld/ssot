...
# Prepend the common install location so sinfo/squeue/sacct/scancel/sbatch all
# work. Override the prepended path with TRAIN_EVAL_REMOTE_PATH_PREPEND (set it
# empty to prepend nothing).
_REMOTE_PATH_PREPEND = os.environ.get("TRAIN_EVAL_REMOTE_PATH_PREPEND", "/opt/slurm/bin")
_SLURM_PATH_PREFIX = (
    f"export PATH={_REMOTE_PATH_PREPEND}:$PATH; " if _REMOTE_PATH_PREPEND else ""
)
